...
(* What `bondi setup` starts the orchestrator with, and what it reads back from
   ...
   The command published a port until this feature, and the address it published
   on was this container's entire security boundary: it mounts the host Docker
   socket, so reaching it is root on the box. It publishes nothing now, and the
   two configuration fields that used to move that address are accepted and
   ignored -- so the cases below are the ones that hold the absence in place,
   because an orchestrator that quietly starts publishing again is the same
   finding as the one that shipped. *)
...
(* The refusal that used to stand here rejected a non-loopback bind_address
   declared without a token, because the orchestrator published an
   unauthenticated Docker-socket API on whatever address that field named. The
   command publishes no port at all now, so the field reaches nothing that can
   expose anything: the configuration that was refused and the configuration
   that declares neither field must produce the same command, byte for byte.
   Asserted as an equality rather than as "no error" -- the equality also fails
   if a residue of the address survives anywhere in the string, which "no
   error" would not. The affirmative arm is taken on the same value first,
   because two empty strings are equal. *)
let test_public_bind_without_a_token_produces_the_ordinary_command () =
  let declared = run_command (cfg ~bind_address:"0.0.0.0" ()) in
  check bool "the value under inspection is the real run command" true
    (contains ~needle:"docker run -d --name bondi-orchestrator" declared);
  check string "a public bind_address changes nothing about the command"
    (run_command (cfg ()))
    declared

(* Both fields are accepted and neither does anything, which is an absence, and
   an absence is satisfied by the empty string -- a builder returning "" would
   pass every line below while establishing nothing. So the affirmative arm is
   taken on the same value first. The token is declared rather than left out,
   because a field nobody sets is trivially never emitted; and its value is
   searched for as well as the flag, so a token reaching the container under
   some other variable name still fails this. No environment flag at all is
   asserted rather than the variable's name, because the name must not survive
   anywhere in this tree. *)
let test_command_names_no_port_no_bind_address_and_no_token () =
  let cmd = run_command (cfg ~bind_address:"0.0.0.0" ~api_token:"s3cret" ()) in
  check bool "the value under inspection is the real run command" true
    (contains ~needle:"docker run -d --name bondi-orchestrator" cmd
    && contains ~needle:"mlopez1506/bondi-server" cmd);
  check bool "publishes no port" false (contains ~needle:"-p " cmd);
  check bool "names no port number" false (contains ~needle:"3030" cmd);
  check bool "names no bind address" false (contains ~needle:"0.0.0.0" cmd);
  check bool "passes no environment variable" false
    (contains ~needle:" -e " cmd);
  check bool "passes no token value" false (contains ~needle:"s3cret" cmd)
...
let test_cron_jobs_imply_root_and_mount () =
  let cmd =
    run_command ~cron_payload_needed:true (cfg ~cron_jobs:[ a_cron_job ] ())
  in
...
let test_cron_jobs_mount_the_payload_directory () =
  ...
  let no_cron = run_command (cfg ()) in
    ...
    [
      ( "run command",
        [
          ...
          test_case "a public bind_address with no token produces a command"
            `Quick
            test_public_bind_without_a_token_produces_the_ordinary_command;
          test_case "no published port, no bind address and no token" `Quick
            test_command_names_no_port_no_bind_address_and_no_token;
        ] );
      ( "verification",
        [
