(** Best-effort outbound delivery of planned alerts over validated TLS.
    ...
    acknowledgement, never its result.
    ...
    regression ([~https:None]) from compiling past this module, and it is where
    the process-wide preconditions an outbound handshake needs — the trust store
    and a seeded random generator — are established. *)
...
(** Build the outbound TLS handler from the operating system's trust store via
    ...
    TLS handler rather than [~https:None]. Also seeds this process's default
    random generator -- before the trust store is read, so it holds on the error
    path too -- which the TLS handshake draws from and which raises until it is
    seeded; the raise would otherwise happen inside a delivery fiber rather than
    here, where nothing a caller writes could tell the two apart. Intended to be
    called once per process, before anything can deliver, and harmless if called
    again. *)
...
(** Whether an HTTP status code counts as a delivered alert: only 2xx. Every
    ...
    meaning to a sink's response than delivered or not. *)
...
(** Best-effort delivery: POST [payload] as JSON to every target in [targets]
    ...
    delivery outcome cannot reach the job path. *)
